...
<head>
    ...
    <style>
        ...
        .main-content { 
            margin-left: 270px; padding: 30px; min-height: 100vh; 
        }
        .header {
        ...
        .header h2 { 
            font-weight: 700; font-size: 26px; color: #333; 
        ...
        .alert {
            ...
            border-radius: 8px;
            font-weight: 600;
            animation: fadeAlert 0.4s ease-in-out;
            position: relative;
                ...
                <tbody>
                    <c:forEach var="u" items="${staffList}" varStatus="i">
                        <tr style="animation-delay:${i.index * 0.05}s;">
                            ...
                            <!-- Vai trò -->
                            <td>
                                <form action="${pageContext.request.contextPath}/admin/staffManagement" method="post" style="display:flex;align-items:center;gap:5px;">
                                    <input type="hidden" name="action" value="updateRole">
                                    <input type="hidden" name="userId" value="${u.userID}">

                                    <select name="roleId" onchange="this.form.submit()">
                                        <c:forEach var="r" items="${roles}">
                                            <c:if test="${r.roleName == 'ADMIN' || r.roleName == 'STAFF'}">
                                                <option value="${r.roleID}" 
                                                    <c:if test="${not empty u.roles and r.roleID == u.roles[0].roleID}">selected</c:if>>
                                                    ${r.roleName}
                                                </option>
                                            </c:if>
                                        </c:forEach>
